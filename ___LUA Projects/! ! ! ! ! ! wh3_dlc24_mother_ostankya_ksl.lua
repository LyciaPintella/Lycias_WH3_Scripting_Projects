...
function mother_ostankya_features_ksl:initialise()
     ...
     if witchs_hut_init == false then
          ...
          for i = 0, faction_cco:Call("CookingSystem.GetPossibleRecipeRecords.Size") - 1 do
               cm:unlock_cooking_recipe(ostankya_faction_obj,
                    faction_cco:Call("CookingSystem.GetPossibleRecipeRecords.At(" .. i .. ").Key"))
          end
     ...
     core:add_listener(
          ...
          "BattleCompleted",
          function()
               return cm:pending_battle_cache_faction_won_battle(self.ostankya_faction) and
               (cm:model():pending_battle():battle_type() ~= "land_normal" or cm:turn_number() > 1)                                                                  -- ignore land battles on turn 1
          end,
          function()
               for ingredient, data in pairs(self.ingredient_unlocks) do
                    if data.races then
     ...
     if ostankya_faction_obj:is_human() then
          ...
          core:add_listener(
               ...
               function(context)
                    ...
                    if unlocked_hex_data then
                         ...
                         cm:complete_scripted_mission_objective(self.ostankya_faction, "wh_main_long_victory",
                              unlocked_hex_data.script_key, true)

                         if count == 3 then -- short victory counter
                              cm:complete_scripted_mission_objective(self.ostankya_faction, "wh_main_short_victory",
                                   "hexes_short_some", true)
                         elseif count == 5 then
                              cm:callback(
                              function() cm:trigger_incident(faction_name, self.hex_6_story_panel_key, true, true) end,
                                   0.2)
          ...
          core:add_listener(
               ...
               function(context)
                    local mission_key = context:mission():mission_record_key()
                    return context:faction():name() == self.ostankya_faction and
                    (mission_key == self.hex_6_ie_mission_key or mission_key == self.hex_6_roc_mission_key)
               end,
               function()
          ...
          function(context)
               ...
               if target_type == "REGION" then
                    ...
                    if is_local_faction then
                         cm:scroll_camera_from_current(true, 1,
                              { settlement:display_position_x(), settlement:display_position_y(), cached_d, cached_b,
                                   cached_h })
                    end

                    cm:add_scripted_composite_scene_to_settlement("wh3_dlc24_campaign_mother_ostankya_hex",
                         "wh3_dlc24_campaign_mother_ostankya_hex", region, 0, 0, true, true, true)
                         ...
                         cm:callback(
                              function()
                                   ...
                                   if character then
                                        if is_local_faction then
                                             cm:scroll_camera_from_current(true, 1,
                                                  { character:display_position_x(), character:display_position_y(),
                                                       cached_d, cached_b, cached_h })
                                        end

                                        cm:add_scripted_composite_scene_to_logical_position(
                                        "wh3_dlc24_campaign_mother_ostankya_hex",
                                             "wh3_dlc24_campaign_mother_ostankya_hex", character:logical_position_x(),
                                             character:logical_position_y(), 0, 0, true, true, true)
                                   end
                              end,
                              0.2
                         )
                    ...
                    if is_table(data.hex) then
                         for i = 1, #data.hex do
                              if data.hex[i] == ritual_key then
                                   incident, incident_received, remove_bundle, persistent_vfx_duration =
                                   get_performed_hex_data(data)
                                   break
                              end
                         end
                    elseif data.hex == ritual_key then
                         incident, incident_received, remove_bundle, persistent_vfx_duration = get_performed_hex_data(
                         data)
                         break
                    end
               end
               ...
               if incident_received and faction_cqi ~= target_faction_cqi then
                    cm:trigger_incident_with_targets(target_faction_cqi, incident_received, 0, 0, character_cqi, 0,
                         region_cqi, 0)
               end
               ...
               if ritual_key:starts_with(self.hex_data.purification_chant.hex) then
                    ...
                    for i = 1, #corruption_types do
                         ...
                         prm:resource("wh3_main_corruption_" .. corruption_types[i]), "local_populace", -100)
                    end
               end
               ...
               if persistent_vfx_duration then
                    if character_cqi > 0 then
                         ...
                         cm:add_turn_countdown_event(cm:get_character_by_cqi(character_cqi):faction():name(),
                              persistent_vfx_duration, "ScriptEventOstankyaCharacterVFXExpires", tostring(character_cqi))
                    elseif region_name then
                         ...
                         cm:add_turn_countdown_event(garrison_residence:faction():name(), persistent_vfx_duration,
                              "ScriptEventOstankyaRegionVFXExpires", region_name)
                    end
               end
          end,
          true
     ...
     core:add_listener(
          ...
          true,
          function(context)
               cm:remove_garrison_residence_vfx(cm:get_region(context.string):garrison_residence():command_queue_index(),
                    "scripted_effect27")
          end,
          true
     )
     ...
     core:add_listener(
          ...
          function(context)
               ...
               if self.ingredient_slot_techs[technology] then
                    cm:set_faction_max_secondary_cooking_ingredients(faction,
                         cm:model():world():cooking_system():faction_cooking_info(faction):max_secondary_ingredients() +
                         1)
               end
          end,
          true
     ...
     core:add_listener(
          ...
          function(context)
               return (context:proposer():name() == self.ostankya_faction or context:recipient():name() == self.ostankya_faction) and
               (context:is_military_alliance() or context:is_defensive_alliance() or context:is_vassalage())
          end,
          function(context)
...
function mother_ostankya_features_ksl:update_hex_spirit_essence_requirements()
     ...
     for hex, data in pairs(self.hex_data) do
          ...
          if data.spirit_essence_requirement then
               common.set_context_value(hex .. "_spirit_essence_remaining",
                    math.max(data.spirit_essence_requirement - spirit_essence_value, 0))
          end
     end
end

function mother_ostankya_features_ksl:get_total_spirit_essence_consumed()
     return cm:get_total_pooled_resource_spent_for_faction(self.ostankya_faction, self.spirit_essence, "hexes") +
     cm:get_total_pooled_resource_spent_for_faction(self.ostankya_faction, self.spirit_essence, "witchs_hut_brews")
end
...
function wh3_dlc24_mother_ostankya_ksl_set_faction(faction, local_faction)
     ...
     count = count + 1
     cm:set_saved_value("hexes_unlocked_count_ksl", count)
     cm:complete_scripted_mission_objective(mother_ostankya_features_ksl.ostankya_faction, "wh_main_long_victory",
          "hexes_victory_5", true)
          ...
          function(context)
               ...
               for i = 0, region_list:num_items() - 1 do
                    ...
                    if region:settlement():get_climate() == "climate_magicforest" then
                         cm:faction_add_pooled_resource(mother_ostankya_features_ksl.ostankya_faction,
                              mother_ostankya_features_ksl.spirit_essence, "magical_forests", 5);
                    end;
               end;
          end,
          true
...
cm:add_saving_game_callback(
     function(context)
          cm:save_named_value("mother_ostankya_features_ksl_faction_name", mother_ostankya_features_ksl.ostankya_faction,
               context)
...
cm:add_loading_game_callback(
     function(context)
          if cm:is_new_game() == false then
               mother_ostankya_features_ksl.ostankya_faction = cm:load_named_value(
               "mother_ostankya_features_ksl_faction_name", "", context)
