	...
	ulrika = {
		...
		mission_chain_keys = {
			main_warhammer = {
				...
				"wh3_dlc23_ie_dwf_ulrika_stage_1",
				"wh3_dlc23_ie_dwf_ulrika_stage_2",
				"wh3_dlc23_ie_brt_ulrika_stage_1",
				"wh3_dlc23_ie_brt_ulrika_stage_2",
				"wh3_dlc23_ie_cth_ulrika_stage_1",
				"wh3_dlc23_ie_cth_ulrika_stage_2",
				"wh3_dlc23_ie_hef_ulrika_stage_1",
				"wh3_dlc23_ie_hef_ulrika_stage_2",
				"wh3_dlc23_ie_wef_ulrika_stage_1",
				"wh3_dlc23_ie_wef_ulrika_stage_2",
				"wh3_dlc23_ie_lzd_ulrika_stage_1",
				"wh3_dlc23_ie_lzd_ulrika_stage_2",
				"wh3_dlc23_ie_tmb_ulrika_stage_1",
				"wh3_dlc23_ie_tmb_ulrika_stage_2"
			},
			wh3_main_chaos = {
...
local out = function(t)
    ModLog("Legendary Characters: " .. tostring(t) .. ".")
end
...
function character_unlocking:setup_mission_listeners(character)
	...
	if character_info.has_spawned == false then
        for i = 1, #character_info.allowed_factions do
			out("setup_mission_listeners: checking faction " .. character_info.allowed_factions[i]:name() .. ".")
				...
				cm:add_faction_turn_start_listener_by_name(
					...
					function(context)
						if character_info.has_spawned == false then
							...
							out("character_info.factions_involved[faction_name]: " .. character_info.factions_involved[faction_name] .. ".")
							if character_info.factions_involved[faction_name] ~= true and faction:faction_leader():rank() >= character_info.unlock_rank then
								out("triggering mission")
...
function character_unlocking:add_quest_mission_listener(character)
	...
	for i = 1, #character_info.allowed_factions do
		...
		out("add_quest_mission_listener faction:name(): " .. faction:name() .. ".")
		if faction and faction:is_human() then
            local faction_name = faction:name()
			out("add_quest_mission_listener character_info.factions_involved[faction_name]: " .. character_info.factions_involved[faction_name] .. ".")
		...
		function(context)
			...
			for i = 1, #character_info.allowed_factions do
				if faction_name == character_info.allowed_factions[i] then
					return character:rank() == character_info.ai_unlock_rank and
		...
		if can_spawn then
			if spawn_character_cqi then
				cm:spawn_unique_agent_at_character(
					cm:get_faction(faction):command_queue_index(),
