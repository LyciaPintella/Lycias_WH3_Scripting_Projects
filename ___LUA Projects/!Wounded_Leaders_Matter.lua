...
local effect_groups = {
    ...
    wh_main_sc_emp_empire = {
        {
            ...
            effect_key = "wh3_main_effect_corruption_vampiric_events_bad",
    ...
    wh2_dlc11_cst_pirates_of_sartosa = {
        {
            effect_key = "wh_main_effect_force_all_campaign_movement_range",
            effect_scope = "faction_to_navy_own",
            value = -25
        }, {
            effect_key = "wh_main_effect_raise_dead_recruitment_cost_mod",
            effect_scope = "faction_to_character_own",
    ...
    wh2_main_def_naggarond = {
        {
            effect_key = "wh_main_effect_force_all_campaign_replenishment_rate",
            effect_scope = "faction_to_force_own",
            value = -4.25
        }, {
            effect_key = "wh_main_effect_agent_action_success_chance",
    ...
    wh2_main_hef_eataine = {
        {
            effect_key = "wh2_main_effect_force_stat_melee_attack_vs_dark_elves",
            effect_scope = "faction_to_force_own",
            value = -5
        }, {
    ...
    wh2_dlc15_hef_imrik = {
        {
            ...
            effect_key = "wh_main_effect_force_all_campaign_recruitment_cost_all",
            effect_scope = "faction_to_force_own",
            value = 15
        }, {
            effect_key = "wh_main_effect_agent_action_success_chance",
    ...
    wh2_dlc13_lzd_spirits_of_the_jungle = {
        {
            effect_key = "wh2_dlc13_lzd_weapon_strength_kroxigor",
            effect_scope = "faction_to_force_own",
            value = -15
        }, {
            effect_key = "wh_main_effect_force_stat_charge_bonus_pct",
    ...
    wh2_main_skv_clan_eshin = {
        {
            ...
            effect_key = "wh2_main_pooled_resource_skaven_food_army",
            effect_scope = "faction_to_force_own",
            value = -1
        }, {
            effect_key = "wh_main_effect_agent_action_success_chance",
    ...
    wh2_main_skv_clan_skyre = {
        {
            effect_key = "wh_main_effect_technology_research_points",
    ...
    wh3_main_sla_seducers_of_slaanesh = {
        {
            ...
            effect_key = "wh3_main_effect_force_all_seduce_units_budget",
            effect_scope = "faction_to_force_own",
            value = -15
        }, {
            effect_key = "wh_main_effect_agent_action_success_chance",
...
local function build_wounded_leader_bundle(target_faction)
    ...
    out("WoundedLeader: Building wounded leader bundle for " .. faction_key ..
            " of subculture " .. sub_key)
    local added_effects = false
    if not default_effects_blacklist[sub_key] and
        (not default_effects_blacklist[faction_key]) then
        ...
        for i = 1, #effect_entries do
            ...
            if is_faction_of_type(target_faction, effect_record.is_human) then
                new_bundle:add_effect(effect_record.effect_key,
                                      effect_record.effect_scope,
                                      effect_record.value)
                added_effects = true
            end
        end
    ...
    if added_effects then
        ...
        out(
            "WoundedLeader: Building Successfully completed and effects applied.")
    end
end
local function check_faction_leader_status(faction)
    if (faction:faction_leader()):is_wounded() and
        (not faction:has_effect_bundle(bundle_key)) then
        out("WoundedLeader: Leader is wounded, building bundles for faction")
        build_wounded_leader_bundle(faction)
    elseif not (faction:faction_leader()):is_wounded() and
        faction:has_effect_bundle(bundle_key) then
        out(
            "WoundedLeader: Leader is no longer wounded, removing bundles for faction")
        cm:remove_effect_bundle(bundle_key, faction:name())
    end
end
local function is_character_attacker(pb, character)
    if character:command_queue_index() == (pb:attacker()):command_queue_index() then
        return true
    end
    for i = 0, (pb:secondary_attackers()):num_items() - 1 do
        if ((pb:secondary_attackers()):item_at(i)):command_queue_index() ==
            character:command_queue_index() then return true end
    end
    return false
end
cm:add_first_tick_callback(function()
    core:add_listener("CheckWoundedFactionLeaders", "FactionTurnStart",
                      function(context)
        return not ((context:faction()):faction_leader()):is_null_interface()
    end, function(context)
        local faction = context:faction()
        local ok, err = pcall(function()
            out("WoundedLeader: Checking faction: " .. faction:name() ..
                    " for wounded leader")
